...
MetricsController = (function(superClass) {
  ...
  function MetricsController(scope, rootscope, repo, rs, params1, q1, location, navUrls, appMetaService, auth, translate, projectService, errorHandlingService, http, urls, $timeout, metricsConfiguration, metricsCustomization) {
    ...
    this.scope.metricsView = {
      ...
      data: null,
      errors: {},
      activeTab: this.location.path().indexOf('/metrics/project') !== -1 ? 'project' : 'team',
      ...
      teamHistoricalMetricOptions: [
        {
          ...
          id: "story_points",
          label: "METRICS.TEAM_HISTORICAL_METRIC_STORY_POINTS"
        }, {
          id: "stories_closed",
          label: "METRICS.TEAM_HISTORICAL_METRIC_STORIES_CLOSED"
        }
      ],
    ...
    this.scope.setActiveTab = (function(_this) {
      return function(tabId) {
        var projectSlug;
        _this.scope.metricsView.activeTab = tabId;
        projectSlug = _this.scope.project.slug;
        if (tabId === 'project') {
          return _this.location.path("/project/" + projectSlug + "/metrics/project");
        } else {
          return _this.location.path("/project/" + projectSlug + "/metrics/team");
        }
      };
    })(this);
    ...
    this.scope.applySprintFilter = (function(_this) {
      return function(targetFilters) {
        return _this.applySprintFilter(targetFilters);
      };
    })(this);
    ...
    promise.then((function(_this) {
      return function() {
        ...
        _this.bootstrapMetricsAccess();
        return _this.loadMilestones();
      };
    })(this));
  ...
  MetricsController.prototype.resolveMetricClassificationValue = function(metric) {
    ...
    if (!classification && metric.classification) {
  ...
  MetricsController.prototype.loadProject = function() {
    var defaultExternal, expectedSlug, project;
    expectedSlug = this.params.pslug;
    if (this.projectService.project) {
      project = this.projectService.project.toJS();
      if (expectedSlug && ((project != null ? project.slug : void 0) != null) && project.slug !== expectedSlug) {
        console.warn("Metrics: Project mismatch, reloading", project.slug, expectedSlug);
        return this.projectService.setProjectBySlug(expectedSlug).then((function(_this) {
          return function() {
      ...
      return this.projectService.setProjectBySlug(expectedSlug).then((function(_this) {
        ...
        return function(err) {
          ...
          _this.scope.metricsView.error = "METRICS.LOAD_ERROR";
      ...
      this.scope.metricsView.loading = false;
      return this.q.reject("Metrics: Missing project slug");
    }
  ...
  MetricsController.prototype.loadMilestones = function() {
    if (!this.scope.projectId) {
      return;
    }
    return this.rs.sprints.list(this.scope.projectId).then((function(_this) {
      return function(data) {
        var i, len, milestones, options, sorted, sprint;
        ...
        _this.scope.metricsView.sprintOptions = options;
        return _this.$timeout(function() {
          _this.scope.metricsView.teamHistoricalFilters.sprint = null;
          _this.scope.metricsView.teamHistoricalFilters.user = "all";
          _this.scope.metricsView.teamHistoricalFilters.metric = "all";
          _this.applySprintFilter(_this.scope.metricsView.teamHistoricalFilters);
          _this.scope.metricsView.projectHistoricalFilters.sprint = null;
          _this.scope.metricsView.projectHistoricalFilters.metric = "all";
          return _this.applySprintFilter(_this.scope.metricsView.projectHistoricalFilters);
        }, 0);
      };
    })(this))["catch"]((function(_this) {
      return function(error) {
        return console.warn("Metrics: Unable to load milestones", error);
      };
    })(this));
  };

  MetricsController.prototype.applySprintFilter = function(filters) {
    var selectedSprint, sprintId;
    if (!filters) {
      return;
    }
    sprintId = filters.sprint;
    selectedSprint = _.find(this.scope.metricsView.sprintOptions, {
      id: sprintId
    });
    if (selectedSprint && selectedSprint.dateFrom && selectedSprint.dateTo) {
  ...
  MetricsController.prototype.loadMetrics = function(force) {
    var externalId, params, projectSlug, ref, url;
    if (force == null) {
      force = false;
    }
    ...
    projectSlug = this.scope.projectSlug || this.params.pslug;
    if (!this.scope.project && this.projectService.project) {
      this.scope.project = this.projectService.project.toJS();
      this.scope.projectId = (ref = this.scope.project) != null ? ref.id : void 0;
    }
    if (!projectSlug) {
      this.scope.metricsView.error = "METRICS.LOAD_ERROR";
    ...
    this.scope.projectSlug = projectSlug;
    ...
    return this.http.get(url, params, {
      ...
      return function(response) {
        var allMetrics, allMetricsData, data, displayMetricGroups, githubUsername, hasHoursData, hoursChart, hoursData, i, j, len, len1, matchesGithub, matchesTaiga, metric, metricId, metricsCategoriesData, normalizedStudents, payloadContext, processedMetrics, processedQualityFactors, processedStrategicIndicators, processedUsers, processedUsersList, projectMetricsList, ref1, ref2, ref3, ref4, student, studentMetrics, studentsClosedTasksBar, studentsOverallRadar, studentsRaw, taigaUsername, transformedPayload, transformedView, viewContext, viewData;
        data = (response != null ? response.data : void 0) || {};
        payloadContext = {
        ...
        if (studentsRaw && angular.isArray(studentsRaw)) {
          for (i = 0, len = studentsRaw.length; i < len; i++) {
            ...
            taigaUsername = (ref1 = student.identities) != null ? (ref2 = ref1.TAIGA) != null ? ref2.username : void 0 : void 0;
            githubUsername = (ref3 = student.identities) != null ? (ref4 = ref3.GITHUB) != null ? ref4.username : void 0 : void 0;
            studentMetrics = [];
      ...
      return function(error) {
        ...
        _this.scope.metricsView.loading = false;
        _this.scope.metricsView.error = _this.resolveErrorKey(error != null ? (ref1 = error.data) != null ? ref1.error : void 0 : void 0, "METRICS.LOAD_ERROR");
  ...
  MetricsController.prototype.loadHistoricalMetrics = function() {
    var externalId, params, projectSlug, ref, url;
    console.log("[DEBUG] loadHistoricalMetrics called, authenticated:", (ref = this.scope.metricsAuth) != null ? ref.authenticated : void 0, "provider:", this.metricsProvider);
      ...
      return function(error) {
        ...
        if (_this.scope.metricsView.data) {
          _this.scope.metricsView.data.historicalMetrics = {
            error: _this.resolveErrorKey(error != null ? (ref1 = error.data) != null ? ref1.error : void 0 : void 0, "METRICS.HISTORICAL_LOAD_ERROR")
          };
        }
        _this.scope.metricsView.teamHistoricalSource = null;
        _this.scope.metricsView.teamHistoricalCharts = [];
  ...
  MetricsController.prototype.prepareProjectMetrics = function(metricsArray) {
    var addMetricEntry, classification, collected, context, finalMetrics, globalHidden, i, isUserMetric, j, k, len, len1, len2, len3, m, metric, metricId, metricsById, normalizedId, normalizedMetricId, projectOrder, ref, ref1, ref2, ref3, seenIds, transformed;
    ...
    if (collected.length === 0 || Object.keys(metricsById).length > collected.length) {
      for (m = 0, len3 = metricsArray.length; m < len3; m++) {
        metric = metricsArray[m];
        if (!(metric != null)) {
          continue;
        }
        if (!((metric.id != null) || (metric.externalId != null))) {
          continue;
        }
        isUserMetric = this.isUserMetricId(metric.id) || this.isUserMetricId(metric.externalId);
        if (isUserMetric) {
  ...
  MetricsController.prototype.buildMetricDisplayGroups = function(rawMetrics) {
    ...
    for (i = 0, len = rawMetrics.length; i < len; i++) {
      ...
      translatedLabel = this.translateMetricId(metric.id);
      displayLabel = translatedLabel || metric.name || this.formatMetricLabel(metric.id);
      entry = {
  ...
  MetricsController.prototype.buildProjectMetricEntry = function(metric) {
    var categoryColor, categoryName, categorySegments, descriptionNumber, displayName, formattedPrecise, normalizedValue, numericValue, percentForColor, preciseValue, ratioValue, rawDescription, ref, roundedValue, translatedName;
    if (!metric) {
      return null;
    }
    normalizedValue = this.normalizeMetricValue(metric.value);
    ...
    translatedName = this.translateMetricId(metric.id);
    displayName = translatedName || metric.name || metric.id;
    return {
      id: metric.id,
      name: displayName,
      description: metric.description || "",
  ...
  MetricsController.prototype.loadInitialData = function() {
    this.scope.metricsView.loading = true;
    return this.q.when(this.loadProject()).then((function(_this) {
      return function(project) {
        return _this.fetchProjectConfig();
      };
  ...
  MetricsController.prototype.convertRawMetricsToCharts = function(metricsData, category) {
    ...
    for (metricKey in metricsData) {
      ...
      if (category === 'project' && this.isUserMetricId(metricKey)) {
        continue;
      }
      labels = [];
      values = [];
  ...
  MetricsController.prototype.identifyHistoricalMetricCategory = function(metricId) {
    ...
    if (normalized === "user_closed_tasks") {
      return "closed_tasks";
    }
    if (normalized === "user_story_points") {
      return "story_points";
    }
    if (normalized === "user_stories_closed") {
  ...
  MetricsController.prototype.composeTeamHistoricalChart = function(collection, metricLabelKey, filteredEntries, showUserInTitle) {
    var baseLabel, chartData, metricLabel, ref, ref1, title, titleKey, translatedLabel;
    if (showUserInTitle == null) {
      showUserInTitle = true;
    }
    if (collection == null) {
      return null;
    }
  ...
  /*
   * Compose simple charts for internal data - one chart per metric showing all users
   */

  MetricsController.prototype.composeSimpleTeamHistoricalCharts = function(chartsByCategory, dateFrom, dateTo) {
    var allDataPoints, allValues, axisConfig, baseLabel, categoryCollections, categoryId, chartData, charts, collection, colorPalette, data, dataByDate, datasets, date, dates, entry, filteredEntries, i, j, k, len, len1, len2, metricLabel, metricLabelKey, point, ref, student, translatedLabel, userLabels;
    charts = [];
    if (chartsByCategory == null) {
      return charts;
    }
    console.log("[DEBUG] composeSimpleTeamHistoricalCharts - categories:", Object.keys(chartsByCategory));
    for (categoryId in chartsByCategory) {
      ...
      for (student in categoryCollections) {
        ...
        for (i = 0, len = filteredEntries.length; i < len; i++) {
        ...
        if (indexOf.call(userLabels, student) < 0) {
          userLabels.push(student);
        }
      }
      if (!allDataPoints.length) {
        continue;
      }
      console.log("[DEBUG] Category", categoryId, "has", allDataPoints.length, "data points from", userLabels.length, "users");
      dataByDate = {};
      for (j = 0, len1 = allDataPoints.length; j < len1; j++) {
        point = allDataPoints[j];
        if (!(point != null)) {
          continue;
        }
        date = point.date;
        ...
        dataByDate[date][point.student] = point.value;
      }
      dates = Object.keys(dataByDate).sort();
      datasets = [];
      for (k = 0, len2 = userLabels.length; k < len2; k++) {
        student = userLabels[k];
        colorPalette = this.resolveUserColor(student);
        data = dates.map(function(date) {
          var ref1, value;
          value = (ref1 = dataByDate[date]) != null ? ref1[student] : void 0;
          if (typeof value === "number" && !isNaN(value)) {
            return value;
          } else {
            return null;
      ...
      allValues = allDataPoints.map(function(p) {
        return p.value;
      }).filter(function(v) {
      ...
      chartData = {
        ...
        yAxisMax: axisConfig.yAxisMax,
        showLegend: true,
        metricCategory: categoryId
      };
      charts.push({
        id: "simple::" + categoryId,
        metric: categoryId,
        metricLabel: metricLabelKey || metricLabel,
        title: metricLabel,
        chartData: chartData
      });
      console.log("[DEBUG] Created simple chart for", categoryId, "with", datasets.length, "datasets");
    }
    console.log("[DEBUG] composeSimpleTeamHistoricalCharts - created", charts.length, "charts");
  ...
  MetricsController.prototype.applyTeamHistoricalFilters = function() {
    var aggregatedCharts, applyOverrides, categoryCollections, categoryId, charts, chartsByCategory, collection, dateFrom, dateTo, filters, isInternalProvider, metric, metricLabelKey, processCollection, ref, ref1, ref2, ref3, ref4, showUserInTitle, simpleCharts, source, student, user, userCollections;
    filters = this.scope.metricsView.teamHistoricalFilters || {};
    source = this.scope.metricsView.teamHistoricalSource;
    console.log("[DEBUG] applyTeamHistoricalFilters - source:", source != null, "chartsByCategory:", Object.keys((source != null ? source.chartsByCategory : void 0) || {}));
    ...
    isInternalProvider = this.metricsProvider === "internal";
    console.log("[DEBUG] applyTeamHistoricalFilters - metric:", metric, "user:", user, "provider:", this.metricsProvider);
    if (user === "all" && metric === "all") {
      if (isInternalProvider) {
        simpleCharts = this.composeSimpleTeamHistoricalCharts(source.chartsByCategory, dateFrom, dateTo);
        console.log("[DEBUG] applyTeamHistoricalFilters - simpleCharts (internal):", simpleCharts != null ? simpleCharts.length : void 0);
        this.scope.metricsView.teamHistoricalCharts = applyOverrides(simpleCharts);
      } else {
        aggregatedCharts = this.composeAggregatedTeamHistoricalCharts(source.chartsByCategory, dateFrom, dateTo);
        console.log("[DEBUG] applyTeamHistoricalFilters - aggregatedCharts (external):", aggregatedCharts != null ? aggregatedCharts.length : void 0);
    ...
    processCollection = (function(_this) {
      return function(collection) {
        ...
        chart = _this.composeTeamHistoricalChart(collection, metricLabelKey, filteredEntries, showUserInTitle);
        if (chart) {
  ...
  MetricsController.prototype.applyProjectHistoricalFilters = function(historicalMetrics) {
    ...
    if (this.scope.metricsView.projectHistoricalMetricOptions.length <= 1) {
      for (metricId in mergedProjectMetrics) {
        if (!hasProp.call(mergedProjectMetrics, metricId)) continue;
        bundle = mergedProjectMetrics[metricId];
        this.scope.metricsView.projectHistoricalMetricOptions.push({
          id: metricId,
          label: bundle.metricLabel || this.formatMetricLabel(metricId)
        });
      }
    }
    filteredData = {};
    ...
    for (normalizedId in mergedProjectMetrics) {
      ...
      if (filters.metric !== "all" && bundle.metricId !== filters.metric) {
        continue;
      }
      dataPoints = bundle != null ? bundle.dataPoints : void 0;
      if (!(angular.isArray(dataPoints) && dataPoints.length)) {
    ...
    if (filters.metric !== "all") {
      ...
      if (chartData) {
        charts.push({
          id: filters.metric,
          title: chartData.title || this.formatMetricLabel(filters.metric),
          chartData: chartData
        });
      }
    } else {
  ...
  MetricsController.prototype.updateTeamHistoricalSource = function(historicalMetrics) {
    var rawUserMetrics, ref, ref1, ref2, teamData;
    console.log("[DEBUG] updateTeamHistoricalSource called, historicalMetrics:", historicalMetrics != null);
    if (historicalMetrics == null) {
    ...
    console.log("[DEBUG] teamData built:", teamData != null ? (ref1 = teamData.users) != null ? ref1.length : void 0 : void 0, "users, chartsByCategory:", Object.keys((teamData != null ? teamData.chartsByCategory : void 0) || {}));
    this.scope.metricsView.teamHistoricalSource = teamData;
    if (teamData != null ? (ref2 = teamData.users) != null ? ref2.length : void 0 : void 0) {
  ...
  MetricsController.prototype.translateMetricId = function(metricId) {
    ...
    var knownPrefixes, normalizedId, parts, ref, translated, translationKey;
    if (!metricId) {
      return null;
    }
    if (((ref = this.translate) != null ? ref.instant : void 0) == null) {
      return null;
    }
    ...
    if (metricId.indexOf('_') !== -1) {
      ...
      if (knownPrefixes.indexOf(parts[0].toLowerCase()) !== -1) {
  ...
  MetricsController.prototype.formatMetricLabel = function(metricId) {
    ...
    var labelMap, parts, prefix, translatedLabel, userMetricMap, words;
    if (!metricId) {
      return metricId;
    }
    translatedLabel = this.translateMetricId(metricId);
