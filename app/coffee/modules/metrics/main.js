...
(function() {
  ...
  MetricsController = (function(superClass) {
    ...
    MetricsController.prototype.loadMetrics = function(force) {
      ...
      return this.http.get(url, params, {
        ...
        return function(response) {
          ...
          console.log("[DEBUG] Before buildMetricDisplayGroups, metrics count:", (data.metrics || []).length, "sample:", (ref1 = data.metrics) != null ? ref1[0] : void 0);
          displayMetricGroups = _this.buildMetricDisplayGroups(data.metrics || []);
          console.log("[DEBUG] After buildMetricDisplayGroups, team groups:", displayMetricGroups != null ? (ref2 = displayMetricGroups.team) != null ? ref2.length : void 0 : void 0);
          ...
          if (studentsRaw && angular.isArray(studentsRaw)) {
            for (i = 0, len = studentsRaw.length; i < len; i++) {
              ...
              taigaUsername = (ref3 = student.identities) != null ? (ref4 = ref3.TAIGA) != null ? ref4.username : void 0 : void 0;
              githubUsername = (ref5 = student.identities) != null ? (ref6 = ref5.GITHUB) != null ? ref6.username : void 0 : void 0;
              studentMetrics = [];
    ...
    MetricsController.prototype.extractMetricCategoryFromId = function(metricId) {
      var categoryMap, lowerId, metricType, parts;
      if (metricId == null) {
        return null;
      }
      lowerId = metricId.toString().toLowerCase();
      if (lowerId.indexOf('user_closed_tasks') === 0 || lowerId.indexOf('closedtasks_') === 0 || lowerId.indexOf('completedtasks_') === 0) {
        return 'closed_tasks';
      }
      if (lowerId.indexOf('user_story_points') === 0 || lowerId.indexOf('totalus_') === 0) {
        return 'story_points';
      }
      if (lowerId.indexOf('user_stories_closed') === 0 || lowerId.indexOf('completedus_') === 0) {
        return 'stories_closed';
      }
      if (lowerId.indexOf('user_commits') === 0 || lowerId.indexOf('commits_') === 0) {
        return 'commits';
      }
      if (lowerId.indexOf('user_assigned_tasks') === 0 || lowerId.indexOf('assignedtasks_') === 0) {
        return 'tasks';
      }
      if (lowerId.indexOf('user_modified_lines') === 0 || lowerId.indexOf('modifiedlines_') === 0) {
        return 'modified_lines';
      }
      if (lowerId.indexOf('tasksratio_') === 0) {
        return 'tasks_ratio';
      }
    ...
    MetricsController.prototype.isUserMetricId = function(metricId) {
      var classification, isNewPattern, isOldPattern, isUserPattern, lowerId, newPatterns, oldPrefixes;
      if (metricId == null) {
        return false;
      }
      lowerId = metricId.toString().toLowerCase();
      newPatterns = ["task_completion_", "tasks_assigned_", "story_points_", "stories_closed_", "commits_count_", "lines_modified_", "code_contributions_"];
      oldPrefixes = ["assignedtasks_", "closedtasks_", "completedtasks_", "commits_", "modifiedlines_", "completedus_", "totalus_", "tasksratio_"];
      isNewPattern = newPatterns.some(function(prefix) {
        return lowerId.indexOf(prefix) === 0;
      });
      isOldPattern = oldPrefixes.some(function(prefix) {
        return lowerId.indexOf(prefix) === 0;
      });
      isUserPattern = isNewPattern || isOldPattern;
      if (isUserPattern) {
        return true;
      }
    ...
    MetricsController.prototype.buildMetricDisplayGroups = function(rawMetrics) {
      var categoryName, classificationOverride, displayLabel, effectiveQualityFactors, entry, factorName, globalHidden, groups, i, isProjectConfigured, isTeamConfigured, isUserMetric, j, k, len, len1, len2, len3, len4, m, metric, n, normalizedExternalId, normalizedMetricId, normalizedValue, pMetric, percentForColor, projectBuckets, projectOrderConfig, projectUnassigned, pushMetric, ratioValue, ref, ref1, ref2, ref3, ref4, tMetric, teamBuckets, teamOrderConfig, teamUnassigned, translatedLabel, userContext;
      console.log("[DEBUG] buildMetricDisplayGroups - provider: " + this.metricsProvider + ", metrics count:", rawMetrics != null ? rawMetrics.length : void 0);
      groups = {
        project: [],
        team: []
      };
      ...
      for (i = 0, len = rawMetrics.length; i < len; i++) {
        ...
        if (this.metricsProvider === "internal" && isUserMetric && metric.id) {
          categoryName = this.extractMetricCategoryFromId(metric.id);
        ...
        if (this.metricsProvider === "internal" && !entry.categoryColor) {
          ...
          entry.categoryColor = this.getInternalGaugeColor(percentForColor, categoryName);
          entry.categoryPalette = this.getInternalGaugePalette(categoryName, metric.id);
        ...
        console.log("[DEBUG] Metric " + metric.id + ": provider=" + this.metricsProvider + ", hasQF=" + ((effectiveQualityFactors != null ? effectiveQualityFactors.length : void 0) > 0) + ", category=" + categoryName + ", isTeam=" + isTeamConfigured + ", isUser=" + isUserMetric + ", user=" + (userContext != null ? userContext.displayName : void 0));
        if (this.metricsProvider === "internal" && isUserMetric && (userContext != null ? userContext.displayName : void 0)) {
        ...
        } else if (this.metricsProvider === "internal" && isUserMetric && (userContext != null ? userContext.username : void 0)) {
        ...
        } else if (this.metricsProvider === "internal" && (!effectiveQualityFactors || effectiveQualityFactors.length === 0) && categoryName) {
          effectiveQualityFactors = [categoryName];
          console.log("[DEBUG] ✓ Auto-grouping metric " + metric.id + " (no QF) into category: " + categoryName);
        } else if ((effectiveQualityFactors != null ? effectiveQualityFactors.length : void 0) > 0) {
          console.log("[DEBUG] ✗ Metric " + metric.id + " keeping qualityFactors:", effectiveQualityFactors);
        }
        if (isProjectConfigured) {
          if (angular.isArray(effectiveQualityFactors) && effectiveQualityFactors.length > 0) {
            for (m = 0, len3 = effectiveQualityFactors.length; m < len3; m++) {
              factorName = effectiveQualityFactors[m];
              if (factorName) {
        ...
        if (isTeamConfigured) {
          if (angular.isArray(effectiveQualityFactors) && effectiveQualityFactors.length > 0) {
            for (n = 0, len4 = effectiveQualityFactors.length; n < len4; n++) {
              factorName = effectiveQualityFactors[n];
              if (factorName) {
    ...
    MetricsController.prototype.convertMetricBucketsToGroups = function(buckets, unassignedList, isTeam) {
      var CATEGORY_PRIORITY, bucketName, label, metricsList, ref, result, sortedFallback, sortedMetrics;
      result = [];
      for (bucketName in buckets) {
        ...
        sortedMetrics = metricsList.slice().sort((function(_this) {
          return function(a, b) {
            var aCat, aLabel, aPriority, bCat, bLabel, bPriority, catCompare, priorityCompare;
            aCat = (a != null ? a.categoryName : void 0) || _this.extractMetricCategoryFromId(a != null ? a.id : void 0) || "";
            bCat = (b != null ? b.categoryName : void 0) || _this.extractMetricCategoryFromId(b != null ? b.id : void 0) || "";
            aPriority = CATEGORY_PRIORITY[aCat] || 999;
            bPriority = CATEGORY_PRIORITY[bCat] || 999;
    ...
    MetricsController.prototype.buildProjectMetricEntry = function(metric) {
      var categoryColor, categoryName, categoryPalette, categorySegments, descriptionNumber, displayName, formattedPrecise, normalizedValue, numericValue, paletteLookupKey, percentForColor, preciseValue, ratioValue, rawDescription, ref, roundedValue, translatedName;
      if (!metric) {
        return null;
      }
      normalizedValue = this.normalizeMetricValue(metric.value);
      ...
      if (this.metricsProvider === "internal") {
        paletteLookupKey = categoryName || metric.id;
        categoryPalette = this.getInternalGaugePalette(paletteLookupKey, metric.id);
        if (!categoryColor && categoryPalette) {
          percentForColor = ratioValue * 100;
          categoryColor = this.getInternalGaugeColor(percentForColor, paletteLookupKey);
        }
      }
      if (this.metricsProvider === "internal" && !categoryColor) {
        percentForColor = ratioValue * 100;
        categoryColor = this.getInternalGaugeColor(percentForColor, categoryName);
      }
    ...
    MetricsController.prototype.prepareQualityFactors = function(factors) {
      var categoryColor, categoryName, categoryPalette, entry, factor, i, len, percentValue, processed, ref, value;
      ...
      for (i = 0, len = factors.length; i < len; i++) {
        ...
        categoryColor = null;
        categoryPalette = null;
        categoryName = factor.name || factor.id;
        if (this.metricsProvider === "internal") {
          categoryColor = this.getInternalGaugeColor(percentValue, categoryName);
          categoryPalette = this.getInternalGaugePalette(categoryName, factor.id);
    ...
    MetricsController.prototype.getInternalGaugePalette = function(categoryName, metricId) {
      var categoryLower, higherIsBetter, isAssignedMetric, metricIdLower, palette;
      if (categoryName == null) {
        categoryName = null;
      }
      if (metricId == null) {
      ...
      categoryLower = categoryName ? categoryName.toString().toLowerCase() : "";
      metricIdLower = metricId ? metricId.toString().toLowerCase() : "";
      isAssignedMetric = metricIdLower.indexOf('assignedtasks') !== -1 || metricIdLower.indexOf('totalus') !== -1 || metricIdLower.indexOf('assignedus') !== -1 || metricIdLower.indexOf('tasksratio') !== -1 || metricIdLower.indexOf('assigned_stories') !== -1 || metricIdLower.indexOf('assignedstories') !== -1 || (categoryLower.indexOf('assigned') !== -1 && categoryLower.indexOf('unassigned') === -1);
      if (isAssignedMetric) {
      ...
      higherIsBetter = true;
      if (categoryLower.indexOf('unassigned') !== -1) {
        if (categoryLower.indexOf('closed') === -1 && categoryLower.indexOf('completed') === -1) {
          higherIsBetter = false;
        }
      }
      palette = higherIsBetter ? [
        {
          value: 33,
          color: 'rgba(239, 68, 68, 0.9)'
        }, {
    ...
    MetricsController.prototype.getInternalGaugeColor = function(percentValue, categoryName) {
      var categoryLower;
      if (categoryName == null) {
        categoryName = null;
      }
      if (categoryName) {
        ...
        if (categoryLower.indexOf('task') !== -1 || categoryLower.indexOf('tareas') !== -1) {
          if (percentValue < 33) {
        ...
        } else if (categoryLower.indexOf('commit') !== -1) {
          if (percentValue < 33) {
            return 'rgba(239, 68, 68, 0.9)';
          } else if (percentValue < 66) {
        ...
        } else if (categoryLower.indexOf('story') !== -1 || categoryLower.indexOf('point') !== -1) {
          if (percentValue < 33) {
            return 'rgba(239, 68, 68, 0.9)';
          } else if (percentValue < 66) {
        ...
        } else if (categoryLower.indexOf('line') !== -1 || categoryLower.indexOf('code') !== -1) {
          if (percentValue < 33) {
            return 'rgba(239, 68, 68, 0.9)';
          } else if (percentValue < 66) {
        ...
        } else if (categoryLower.indexOf('hour') !== -1 || categoryLower.indexOf('time') !== -1 || categoryLower.indexOf('hora') !== -1) {
          if (percentValue < 33) {
            return 'rgba(239, 68, 68, 0.9)';
          } else if (percentValue < 66) {
    ...
    MetricsController.prototype.buildChartDatasetFromEntries = function(student, category, entries) {
      var avgValue, axisConfig, b, categoryColorRgba, colorPalette, dataset, entry, g, i, labels, len, numericValue, r, rgbaMatch, sum, values;
      ...
      colorPalette = this.resolveUserColor(student);
      if (this.metricsProvider === "internal" && category) {
        avgValue = 0;
        if (values.length > 0) {
        ...
        categoryColorRgba = this.getInternalGaugeColor(avgValue, category);
        if (categoryColorRgba && typeof categoryColorRgba === 'string') {
    ...
    MetricsController.prototype.identifyHistoricalMetricCategory = function(metricId) {
      ...
      if (normalized === "user_commits") {
        return "commits";
      }
      if (normalized === "user_assigned_tasks") {
        return "tasks";
      }
      if (normalized === "user_modified_lines") {
        return "modified_lines";
      }
      return null;
    };
    ...
    MetricsController.prototype.metricCategoryLabelKey = function(category) {
      switch (category) {
        ...
        case "story_points":
          return "METRICS.TEAM_HISTORICAL_METRIC_STORY_POINTS";
        case "stories_closed":
          return "METRICS.TEAM_HISTORICAL_METRIC_STORIES_CLOSED";
        default:
          return null;
      }
    };
    ...
    MetricsController.prototype.composeSimpleTeamHistoricalCharts = function(chartsByCategory, dateFrom, dateTo) {
      var categoryCollections, categoryId, chart, charts, collection, filteredEntries, metricLabelKey, student;
      charts = [];
      ...
      for (categoryId in chartsByCategory) {
        ...
        for (student in categoryCollections) {
          ...
          chart = this.composeTeamHistoricalChart(collection, metricLabelKey, filteredEntries, true);
          if (chart) {
            charts.push(chart);
      ...
      charts.sort(function(a, b) {
        var labelA, labelB, metricCompare;
    ...
    MetricsController.prototype.applyTeamHistoricalFilters = function() {
      var aggregatedCharts, applyOverrides, categoryCollections, categoryId, charts, chartsByCategory, collection, currentProvider, dateFrom, dateTo, filters, isInternalProvider, metric, metricLabelKey, processCollection, ref, ref1, ref2, ref3, ref4, ref5, ref6, showUserInTitle, simpleCharts, source, student, user, userCollections;
      ...
      currentProvider = ((ref = this.localConfig) != null ? ref.provider : void 0) || ((ref1 = this.scope.config) != null ? ref1.provider : void 0) || this.metricsProvider || "external";
      isInternalProvider = currentProvider === "internal";
      ...
      if (user === "all") {
        if (metric === "all") {
          ref2 = source.chartsByCategory;
          for (categoryId in ref2) {
            if (!hasProp.call(ref2, categoryId)) continue;
            categoryCollections = ref2[categoryId];
            for (student in categoryCollections) {
              if (!hasProp.call(categoryCollections, student)) continue;
              collection = categoryCollections[student];
          ...
          chartsByCategory = ((ref3 = source.chartsByCategory) != null ? ref3[metric] : void 0) || {};
        ...
        if (metric === "all") {
          userCollections = ((ref4 = source.chartsByUser) != null ? ref4[user] : void 0) || {};
          ...
          collection = (ref5 = source.chartsByUser) != null ? (ref6 = ref5[user]) != null ? ref6[metric] : void 0 : void 0;
          if (collection) {
            processCollection(collection);
          }
        }
